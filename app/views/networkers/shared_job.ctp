<script>
	...
	function onSelectChange(){
	    ...
	    if(selected.val() != 0){
			window.location.href="/networkers/sharedJob/display:"+selected.text();
	    }
	}
	function onSelectShortByChange(){
	    var selected = $("#short_by option:selected");    
	    if(selected.val() != 0){
			window.location.href="/networkers/sharedJob/shortby:"+selected.val();
	    }
	}	
</script>
                        ...
                        <h2>
                        	<?php
								if($job['Job']['is_active'] == 1){
									...
									echo $this->Html->link(strtoupper($job['Job']['title']),"/jobs/jobDetail/".$job['Job']['id']);
								}else if($job['Job']['is_active'] == 0 || $job['Job']['is_active'] == 2){
									$status= "<span style='float:right;'><b>[Closed]</b></span>";
									echo strtoupper($job['Job']['title']);
								}
							?>
						</h2>
                        <p><?php	
